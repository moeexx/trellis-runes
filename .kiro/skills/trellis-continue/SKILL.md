...
- `status=planning` → stay in Phase 1; plan via `trellis-brainstorm`, request review, then run `task.py start` (gate-checked).
...
```bash
python3 ./.trellis/scripts/get_context.py --mode phase --step <X.X> --platform kiro
```
